...
export default function ShowtimeDetail(props) {
        ...
        label: (
          <div className="text-left border-b pb-4">
            <h2 className="text-green-500 font-bold text-base">
              {itemRap.tenCumRap.length > 35
                ? itemRap.tenCumRap.slice(0, 35) + '...'
                : itemRap.tenCumRap}
            </h2>
            <h3 className="text-gray-500 font-semibold text-sm">
              {itemRap.diaChi.length > 36
                ? itemRap.diaChi?.slice(0, 36) + '...'
                : itemRap.diaChi}
            </h3>
          </div>
        ),
  ...
  const renderHeThongRap = () => {
    ...
    heThongRapChieu.forEach((itemHeThongRap, iHeThong) => {
      heThongRap.push({
        label: <img height={50} width={50} src={itemHeThongRap.logo}></img>,
        key: iHeThong,
        children: (
          <Tabs
            tabPosition="left"
    ...
    <>
      {heThongRapChieu.length ? (
        <>
          ...
          <div
            id="showtime"
            className="container hidden lg:block bg-white showtimeTab mb-8 mt-24 scroll-mt-[11rem]"
          >
            <Tabs
          ...
          <div className="container block lg:hidden mt-4">
            {heThongRapChieu?.map((itemHeThongRap) =>
              itemHeThongRap.cumRapChieu?.map((itemCumRap, iCumRap) => (
                <div key={iCumRap}>
                  <h2 className="text-white bg-orange-400 my-2">
        ...
        <h2 className="text-white text-center my-6 text-2xl">
