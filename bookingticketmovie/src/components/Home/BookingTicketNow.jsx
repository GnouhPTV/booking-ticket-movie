    ...
    if (e.target.value == 'Phim') {
      ...
      const callApiLichChieuTheoPhim = async (value) => {
        try {
          ...
          setArrCumRapSearch(
            layDanhSachCumRap(apiLichChieu.data.content.heThongRapChieu)
          );
        } catch (error) {
          console.log(error);
        }
  ...
  return (
    <div className=" bg-white rounded-lg shadow-2xl text-white py-7 px-8 w-full xl:w-3/4 mx-auto translate-y-[-50%] hidden md:block">
      <div className="grid md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7 gap-2">
        <select
          onChange={handleChangeFilmSearch}
        ...
        >
          ...
          {arrLichChieuPhimSearch?.map((item, index) => (
        ...
        <button
          ...
          className="p-2 bg-orange-400 rounded-md font-semibold tracking-wide h-[2.5rem]"
        >
          Đặt Vé Nhanh
        </button>
      </div>
