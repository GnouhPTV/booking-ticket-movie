...
export default function BookingTicketNow(props) {
    ...
    if (name == 'phimDangChon') {
      const callApiLichChieuTheoPhim = async (value) => {
        try {
          ...
          danhSachDuLieu.rap = layDanhSachCumRap(
            apiLichChieu.data.content.heThongRapChieu
          );
  ...
  const handleOnSubmit = (e) => {
    e.preventDefault();
    if (
      state.lichChieuDangChon !== '' &&
      state.lichChieuDangChon !== 'Ngày giờ chiếu'
    ) {
  ...
  return (
    <div className=" bg-white rounded-lg shadow-2xl text-white py-7 px-8 w-full xl:w-3/4 mx-auto translate-y-[-50%] hidden md:block">
      <form
        ...
        className="grid md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7 gap-2"
      >
        <select
          name="phimDangChon"
          onChange={handleChange}
          className="text-black border-2 rounded-md border-slate-600 cursor-pointer 2xl:col-span-2 h-[2.5rem]"
        >
        ...
        >
          ...
          {state.danhSachDuLieu.rap?.map((item, index) => {
            return (
              <option key={index} value={JSON.stringify(item.lichChieuPhim)}>
                {item.tenCumRap}
              </option>
            );
          })}
