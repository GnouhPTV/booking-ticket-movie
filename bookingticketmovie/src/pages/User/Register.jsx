...
export default function Register() {
  ...
  const HandleChangeInput = (e) => {
    ...
    if (name == 'taiKhoan') {
      isValid &=
        kiemTraRong(value, err, name, title) &&
        kiemTraDinhDang(
          value,
          err,
          name,
          title,
          /^\S*$/,
          'không được có khoảng cách'
        );
    }
    if (name == 'matKhau') {
      isValid &=
        kiemTraRong(value, err, name, title) &&
        kiemTraDoDai(value, err, name, title, 6, 50) &&
        kiemTraDinhDang(
          value,
          err,
          name,
          ...
          'phải có chữ thường, chữ hoa, số và ký tự đặc biệt'
        );
    }
    if (name == 'hoTen') {
      isValid &=
    ...
    if (name == 'soDt') {
      isValid &=
        kiemTraRong(value, err, name, title) &&
        kiemTraDinhDang(value, err, name, title, /^[0-9]+$/, 'phải là số') &&
        kiemTraDoDai(value, err, name, title, 10, 10);
    }
  ...
  return (
    <div className="register mt-16">
      ...
      <form
        ...
        className="form rounded-lg bg-white p-2 sm:px-8 sm:py-4 lg:py-6"
      >
        ...
        <div className="text-right">
          <span
            ...
            className="text-black hover:text-black font-medium cursor-pointer"
          >
            Bạn đã có tài khoản ?{' '}
            <span className="text-red-600">Đăng nhập ngay !</span>
          </span>
        </div>
      </form>
    </div>
